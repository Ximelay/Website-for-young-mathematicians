...
@section('title', 'Муниципальный этап — ' . $stage->municipality->name)
...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500 mb-6">
        ...
        <span>/</span>
        <span class="text-gray-900 font-medium">{{ $stage->municipality->name }}</span>
    </nav>
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <div class="flex items-center gap-2 flex-wrap">
                        <h1 class="text-2xl font-bold text-gray-900">Муниципальный этап</h1>
                    ...
                    <p class="text-lg text-gray-600 mt-0.5">{{ $stage->municipality->name }}</p>
                </div>
            </div>
            @auth
                @if(auth()->user()->hasRole('organizer'))
                    <div class="flex items-center gap-2 flex-shrink-0">
                        <a href="{{ route('stages.municipal.edit', $stage) }}"
                           class="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition shadow-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                            </svg>
                            Редактировать
                        </a>
                        <form method="POST" action="{{ route('stages.municipal.destroy', $stage) }}">
                            @csrf @method('DELETE')
                            <button type="submit"
                                    onclick="return confirm('Удалить этот этап?')"
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="px-6 py-4 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition">
                        <div class="flex items-center justify-between gap-4">
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <a href="{{ route('teams.show', $team) }}"
                                       class="text-sm font-semibold text-gray-900 hover:text-blue-600 transition truncate block">
                                        {{ $team->name }}
                                    </a>
                            ...
                            <div class="flex items-center gap-3 flex-shrink-0">
                                ...
                                {{-- Квалификация --}}
                                @if($team->pivot->qualified_for_regional)
                                    ...
                                    @if(auth()->user()->hasRole('organizer'))
                                        <div x-data="{ open: false }" class="relative">
                                            ...
                                            <div x-show="open" x-transition
                                                 class="absolute right-0 mt-1 w-64 bg-white rounded-xl shadow-lg border border-gray-100 z-10 p-4 space-y-3"
                                                 style="display:none">
                                                <form method="POST" action="{{ route('stages.municipal.update-team', [$stage, $team]) }}" class="space-y-2">
                                                    @csrf @method('PUT')
                                                    ...
                                                    <label class="flex items-center gap-2 text-sm cursor-pointer">
                                                        <input type="hidden" name="qualified_for_regional" value="0">
                                                        <input type="checkbox" name="qualified_for_regional" value="1"
                                                               {{ $team->pivot->qualified_for_regional ? 'checked' : '' }}
                                                               class="rounded border-gray-300 text-green-600">
                                                        <span class="text-gray-700">Прошла в региональный</span>
                                                    </label>
                                                    <button type="submit"
                                                            class="w-full py-1.5 bg-blue-600 text-white text-xs font-semibold rounded-lg hover:bg-blue-700 transition">
                                                        Сохранить
                                                    </button>
                                                </form>
                                                <form method="POST" action="{{ route('stages.municipal.remove-team', [$stage, $team]) }}">
                                                    @csrf @method('DELETE')
                                                    <button type="submit"
                ...
                @empty
                    <div class="flex flex-col items-center justify-center py-14 text-center px-6">
                        ...
                        <p class="text-gray-400 text-xs mt-1">Добавьте команды из панели слева</p>
                    </div>
                @endforelse
            </div>
        </div>
