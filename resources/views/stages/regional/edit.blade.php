...
@section('title', 'Редактирование регионального этапа ' . $stage->year)
...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500 mb-6">
        ...
        <span>/</span>
        <a href="{{ route('stages.regional.show', $stage) }}" class="hover:text-indigo-600 transition">Региональный {{ $stage->year }}</a>
        <span>/</span>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100">
            <h1 class="text-xl font-bold text-gray-900">✏️ Редактирование регионального этапа</h1>
        </div>

        <form method="POST" action="{{ route('stages.regional.update', $stage) }}" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="number" name="year" value="{{ old('year', $stage->year) }}" required min="2000" max="2100"
